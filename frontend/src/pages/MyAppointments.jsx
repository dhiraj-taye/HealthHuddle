...
const MyAppointments = () => {
  ...
  const bookAppointmentStripe = async (appointment) => {
    try {
      const { userId, docId, slotDate, slotTime, fees, userData } = appointment;
        ...
        {appointments.map((item, index) => (
          ...
          >
            ...
            <div className="flex flex-col gap-2 justify-end">
              ...
              {!item.cancelled && !item.payment && !item.isCompleted && (
                <button
                  onClick={() => bookAppointmentStripe(item)}
              ...
              {!item.cancelled && !item.isCompleted && (
                <button
                  onClick={() => cancelAppointment(item._is)}
                  className="sm:min-w-48 py-2 border rounded text-stone-500 bg-indigo-50"
                >
                  Cancel appointment
                </button>
